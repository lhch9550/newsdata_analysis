...
한국언론진흥재단이 개발한 kpf-BERT를 기반으로, 신문기사에서 150개 클래스를 분류하는 NER 모델인 kpf-BERT-ner를 개발한다. 이 모델은 문장의 각 단어에 적절한 태그를 부여하는 다중 분류 작업을 수행하며, 모두의 말뭉치의 국립국어원 신문 말뭉치를 데이터로 사용한다. 또한 kpf-BERT 토크나이저로 문장을 토큰화하고, 특수 토큰들을 추가한 후 BIO 표기법을 적용해 개체의 시작, 내부, 비개체를 구분하여 높은 정확도를 달성한다.
