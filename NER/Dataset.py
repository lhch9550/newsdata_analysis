...
def load_data(data, tokenizer, max_length: int = 128):
    ...
    for i in range(len(data['document'])):
        for j in range(len(data['document'][i]['sentence'])):
            ...
            char_labels = ["O"] * len(sentence)  # 문장 길이만큼 "O"로 초기화

            # BIO 표기법 적용
            for k in range(len(data['document'][i]['sentence'][j]['NE'])):
                begin = data['document'][i]['sentence'][j]['NE'][k]['begin']
                end = min(data['document'][i]['sentence'][j]['NE'][k]['end'], len(sentence))  # 문장 길이를 넘지 않도록 제한
                entity_label = data['document'][i]['sentence'][j]['NE'][k]['label']

                # 시작 위치가 문장 길이보다 작은 경우에만 처리
                if begin < len(sentence):
                    # ✅ BIO 태깅 적용
                    for q in range(begin, end):
                        if q == begin:
                            char_labels[q] = f"B-{entity_label}"
                        else:
                            char_labels[q] = f"I-{entity_label}"

            # 토큰화 및 offset_mapping 처리
            encoding = tokenizer(sentence, max_length=max_length, return_offsets_mapping=True, truncation=True)
